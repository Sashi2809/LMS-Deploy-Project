...
export const getRecommendedCourses = async (req, res) => {
  try {
    ...
    // Recommend courses that are published and not purchased by the user
    const recommendedCourses = await Course.find({
      isPublished: true,
      _id: { $nin: purchasedCourseIds }, // Exclude purchased courses
    })
      .populate({ path: "creator", select: "name photoUrl" })
      .sort({ enrolledStudents: -1 }) // Sort by popularity
      .limit(4);
...
export const createComment = async (req, res) => {
  try {
    ...
    const comment = await Comment.create({ content, course: courseId, user: userId });
